...
DROP FUNCTION IF EXISTS public.prevent_last_admin_deletion();
DROP FUNCTION IF EXISTS public.prevent_last_admin_role_change();

-- Replace existing policies with RBAC policies
DROP POLICY IF EXISTS "Users can insert their own profile" ON public.profiles;
DROP POLICY IF EXISTS "Only admins can create profiles" ON public.profiles;

-- Create new policy: Only admins can create profiles for other users, users can create their own during signup
...
WITH CHECK (
  ...
  (role = (SELECT p.role FROM public.profiles p WHERE p.user_id = user_id) OR has_role(auth.uid(), 'admin'::user_role))
);

-- Replace admin update policy
DROP POLICY IF EXISTS "Admins can update all profiles" ON public.profiles;
DROP POLICY IF EXISTS "Admins can update all profiles with role restrictions" ON public.profiles;
...
WITH CHECK (
  ...
  -- Admins can only assign admin or user roles
